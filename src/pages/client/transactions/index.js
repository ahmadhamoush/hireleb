              ...
              {transactions.received.map((receive, index) => {
                return (
                  <div key={index} className={style.transactions}>
                    ...
                    <h5>
                      <span>Received on </span> {receive.date}
                    </h5>
                  </div>
                )
              })}
              ...
              {transactions.deposits.map((deposit, index) => {
                return (
                  <div key={index} className={style.transactions}>
                    ...
                    <h5>
                      <span>Deposited on </span> {deposit.date}
                    </h5>
                  </div>
                )
              })}
              ...
              {transactions.transfers.map((transfer, index) => {
                return (
                  <div key={index} className={style.transactions}>
                    ...
                    <h5>
                      <span>Transfered on </span> {transfer.date}
                    </h5>
                  </div>
                )
              })}
  ...
  return {
    props: {
      transactions: JSON.parse(
        JSON.stringify(await getTransactions((await session).user.email)),
      ),
    },
  }
}
